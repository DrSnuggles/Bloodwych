function loadBinaryFiles(item, res){
  loadingScreen(item);
  if (typeof item.towerData !== 'undefined'){
    var type = item.id.split("_")[1];
    getFileData(item.callback,item.id,getTowerByName(item.towerData),type,parseInt(item.length), res);
  }else{
    getFileData(item.callback,item.id,null,item.id,parseInt(item.length), res);
  }
}

function getFileData(callback, id, t, type, length, res) {
  if (debug){
  ...
  switch (type) {
    case "monsterPalette":
      monsterPaletteData = window[callback](res, length);
      break;
    case "monsterPaletteMeta":
      monsterPaletteMetaData = window[callback](res, length);
      break;
    case "monsterHeads":
      monsterHeadsData = window[callback](res, length);
      break;
    case "monsterBodies":
      monsterBodiesData = window[callback](res, length);
      break;
    case "map":
      t.floor = window[callback](res, length);
      break;
    case "switches":
      t.switches = window[callback](res, length);
      break;
    case "triggers":
      t.triggers = window[callback](res, length);
      break;
    case "monsters":
      t.monsterData = window[callback](res, length);
      break;
    case "championData":
      championData = window[callback](res, length);
      break;
    case "championPocketData":
      championPocketData = window[callback](res, length);
      break;
    case "towerSwitchesData":
      towerSwitchesData = window[callback](res, length);
      break;
    case "objects":
      t.itemData = window[callback](res, length);
      break;
    case "scrollData":
      scrollData = window[callback](res);
      break;
    case "gemSwitchesData":
      gemSwitchesData = window[callback](res, length);
      break;
    case "crystalSwitchesData":
      crystalSwitchesData = window[callback](res, length);
      break;
    case "armourData":
      armourData = window[callback](res, length);
      break;
    case "monsterItemData":
      monsterItemData = window[callback](res, length);
      break;
    default:
      break;
  }
